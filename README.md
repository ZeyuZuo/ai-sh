...
`ai-sh` 是一个面向开发者和运维人员的命令行助手。你描述想做什么，它会结合当前目录、shell、操作系统和常用工具生成一条 shell 命令，解释它的作用，标记风险，并且只在你明确确认后执行。
...
2. 本地正则和参数解析会在执行前硬拦截危险命令。
3. `danger` 命令直接拒绝，不进入确认流程。
4. `caution` 命令需要用户明确确认。
5. 任何命令都没有自动执行模式。
