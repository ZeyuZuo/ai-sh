...
**状态：** 阶段 1-6 已完成；统一特殊指令已实现；旧 REPL 和执行器已删除
**目标版本：** v0.2  
**决策日期：** 2026-07-11  
...
`[y]` 执行、`[e]` 编辑、`[n]` 取消和数字选择备选命令，本质上是在重复实现 Shell 已有的行编辑、命令历史和光标操作。用户需要学习一套只在 tmksh 内有效的交互。

### 2.3 REPL 形成“终端里的另一个终端”

默认启动 tmksh REPL 后，用户容易混淆当前输入是在真实 Shell 还是 tmksh 会话中。退出、命令历史、快捷键和执行上下文也因此出现两套规则。
...
`git diff | tmksh "总结改动"` 需要返回自然语言答案，而不是 Shell 命令。v0.1 的模型响应要求必须返回命令或澄清问题，无法准确表达纯文本答案。
...
tmksh 返回一行说明，并将命令放入当前 Shell 输入缓冲区：
...
如果缺少必要范围或路径，tmksh 不修改当前缓冲区，而是在同一次 Widget 交互内提出澄清问题：
...
```bash
git diff | tmksh ask "总结这次改动"
cat error.log | tmksh ask "分析报错原因和修复方向"
```
...
Shell Widget 的 `tmksh>` 提示符统一支持 `/fix`、`/explain`、`/check`、`/new`、`/ask` 和 `/help`。`/help` 与未知指令由本地处理，不调用模型；未知指令会返回相近指令建议。

`/explain` 和 `/check` 优先分析当前 buffer，buffer 为空时回退到上一条 Shell 命令；`/new` 明确忽略当前 buffer；`/ask` 只返回文本。解释、检查、问答和帮助结果都不修改原 buffer 或 cursor。

### 4.8 无 Shell 集成时的降级方式
...
```text
tmksh <request>                 生成人类可读的命令建议，不执行
tmksh ask <question>            回答问题，可读取 stdin
tmksh --json <request>          输出稳定的机器可读结果

tmksh config                   配置 API
tmksh config --show            查看脱敏后的配置状态
tmksh init zsh                 输出 zsh Widget 初始化脚本
...
生成的 Bash/Zsh/Fish Widget 使用 `request<NUL>buffer<NUL>failed_command<NUL>exit_code<NUL>cwd<NUL>shell<NUL>last_command` 作为内部 Shell 安全传输帧，并调用 `tmksh suggest --input-format nul`。没有失败记录时第 3 至 6 个字段为空，没有上一条命令时第 7 个字段为空。后端同时接受旧 Widget 的 2 字段和 6 字段帧；新增字段追加在旧 6 字段之后，因此 `protocol_version` 仍为 `1`。公开机器接口默认读取下方的版本化 JSON，两种输入方式共用相同长度限制、结果协议和退出码。

升级后，已经加载在 Shell 进程中的旧 Widget 需要重新执行 `eval "$(tmksh init bash)"`、`eval "$(tmksh init zsh)"` 或 `tmksh init fish | source`，也可以重新打开已配置的 Shell。旧脚本不会发送 `last_command`，也没有统一的文本 `answer` 分派逻辑。
...
Widget 调用 `tmksh suggest` 时，通过 stdin 发送一个 UTF-8 JSON 对象：
...
`failed_command` 是可选字段。普通请求和旧客户端可以省略它；`/fix` 缺少该字段时在本地返回缺少上下文，不调用 API。

`last_command` 也是可选字符串字段，用于当前 buffer 为空时为 `/explain` 和 `/check` 提供回退目标。例如：

```json
{
  "protocol_version": 1,
  "request": "/explain 为什么需要 -print0",
  "buffer": "",
  "last_command": "find . -type f -print0 | xargs -0 du -h"
}
```

后端只会在 `/explain` 或 `/check` 选择它作为目标时把该命令发送给模型，其他指令不会把 `last_command` 传给模型。
...
- `failed_command.command` 不超过 32768 字符，`cwd` 不超过 4096 字符，`shell` 不超过 64 字符，`exit_code` 必须为正整数。
- `last_command` 必须是字符串，不超过 32768 字符。
- `tmksh --json` 使用相同的 request 限制；其管道上下文最多读取 65536 字符并明确标记截断。
...
- 非 `command` 响应必须清空 `command`；Widget 只在退出码为 0 且 `kind=command` 时修改 buffer。
...
- tmksh 只记录生成请求、建议命令、风险结果和是否被插入，不记录“已执行”，因为 tmksh 不再知道用户是否按了 Enter。
- 问答 stdin 默认不持久化。
- REPL 输入历史文件和 tmksh 历史文件权限必须为 `600`。
- 当前 buffer 只发送到用户配置的 API endpoint。
- `/fix` 只额外发送失败命令、退出码、执行目录和 Shell 类型，不捕获或上传 stdout/stderr。
- 上一条命令保存在当前 Shell 中，只有空 buffer 的 `/explain` 或 `/check` 将其作为分析目标发送给配置的 API endpoint。
...
- 将本地安全判断合并进统一结果规范化流程。
- 修改 `tmksh <request>`，只展示命令，不调用 executor。
...
- 所有 `tmksh` 调用都不会触发 `subprocess.run`。
...
- 实现 `tmksh init bash`，通过 Readline buffer 完成首个可用版本。
- 实现 `tmksh init zsh`，复用相同后端协议和交互语义。
- 注册可配置快捷键，默认 `Ctrl+G`。
...
**实施状态：已完成。** `tmksh ask` 使用独立纯文本提示词和解析路径；stdin 按字节流式读取，保留上限为 64 KiB，超限会同时向用户和模型标记截断。该模式不调用命令建议、安全归一化、执行器或持久化历史。
...
- `git diff | tmksh ask "总结改动"` 返回文本答案，不生成命令。
- 问答失败时返回非零退出码和用户可理解的错误。
- stdin 内容不写入持久化历史。

### 阶段 5：支持 fish 并加固跨 Shell 兼容性

**实施状态：已完成。** Fish commandline Widget 已复用相同协议、安全语义和失败恢复行为，CI 会安装 Fish 和 Zsh 后运行跨 Shell 测试。

目标：在 bash 和 zsh MVP 稳定后补齐 fish，并覆盖 PRD 声明支持的三个 Shell。

任务：

- 实现 `tmksh init fish` 和 fish commandline 替换。
...
**实施状态：已完成。** CI 覆盖 Python 3.10/3.14、pytest、Ruff、首批 strict mypy、构建和 wheel 安装 smoke test；LLM 兼容错误路径已使用 mock 覆盖。
