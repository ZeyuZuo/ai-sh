...
生成的 Bash/Zsh/Fish Widget 使用 `request<NUL>buffer<NUL>failed_command<NUL>exit_code<NUL>cwd<NUL>shell` 作为内部 Shell 安全传输帧，并调用 `tmksh suggest --input-format nul`；没有失败记录时后四个字段为空。后端仍兼容旧 Widget 的 `request<NUL>buffer` 双字段帧。公开机器接口默认读取下方的版本化 JSON，两种输入方式共用相同长度限制、结果协议和退出码。
