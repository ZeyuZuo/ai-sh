...
生成的 Bash/Zsh Widget 使用 `request<NUL>buffer` 作为内部 Shell 安全传输帧，并调用 `tmksh suggest --input-format nul`；后端随即转换为同一个 `ProtocolRequest`。公开机器接口仍默认读取下方的版本化 JSON。两种输入方式共用相同长度限制、结果协议和退出码。
